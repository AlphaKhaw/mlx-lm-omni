...
class ModelArgs:
    ...
    def from_dict(cfg: dict) -> "ModelArgs":
        ...
        return ModelArgs(thinker_config=cfg["thinker_config"])
...
class Model(nn.Module):
    ...
    def build_custom_tokenizer(self, tokenizer: TokenizerWrapper) -> ExtendedTokenizer:
        return TokenizerWithAudio(
            self.thinker.audio_tower, tokenizer, self.thinker.model.embed_tokens
        )
...
class TokenizerWithAudio(ExtendedTokenizer):
    def __init__(
        ...
        embeddings: ExtendedEmbedding,
    ):
        self._audio_mel = AudioMel(
            AudioMelConfig.from_dict(
                {
                    "feature_size": 128,
                    "sampling_rate": 16000,
                    "hop_length": 160,
        ...
        for message in messages:
            if message.get("audio", None) is not None:
                ...
                if AUDIO_SPECIAL_TOKEN not in message["content"]:
                    message["content"] += (
                        f"<|audio_bos|>{AUDIO_SPECIAL_TOKEN}<|audio_eos|>"
                    )
                elif (
                    f"<|audio_bos|>{AUDIO_SPECIAL_TOKEN}<|audio_eos|>"
                    ...
                    message["content"] = message["content"].replace(
                        AUDIO_SPECIAL_TOKEN,
                        f"Audio 1: <|audio_bos|>{AUDIO_SPECIAL_TOKEN}<|audio_eos|>",
                    )
